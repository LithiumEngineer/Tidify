...
from services.file_utils import is_image, move_to_trash, log_action
...
class DedupService:
    def __init__(self, directory: str, sensitivity: int = 90):
        ...
        self._progress: dict = {"phase": "pending", "current": 0, "total": 0, "message": "Waiting..."}
        self.done = False
    ...
    def _run(self) -> None:
        try:
            ...
            if not emb_service.ensure_model(on_progress=lambda msg: self._set_progress("scanning", 0, 0, msg)):
    ...
    def _compute_embeddings(self, emb_service) -> None:
        ...
        for i, path in enumerate(self._image_paths):
            try:
                ...
                vectors.append(np.zeros_like(vectors[0]) if vectors else np.zeros(1000, dtype=np.float32))
                failed += 1
    ...
    def _rank_groups(self) -> None:
        ...
        for idx, group_indices in enumerate(self._raw_groups.values()):
            ...
            if (idx + 1) % 10 == 0 or idx == total_groups - 1:
                self._set_progress(
                    "ranking", idx + 1, total_groups,
                    f"Ranked {idx + 1}/{total_groups} groups"
                )
